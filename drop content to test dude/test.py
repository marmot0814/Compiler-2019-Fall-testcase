...
class Grader:
    ...
    def gen_output(self, case_id):
        ...
        try:
            ...
            print(Colors.RED + "Call of '%s' failed: %s" %
                  (" ".join(clist), e))
            exit(1)
    ...
    def compare_file_content(self, case_id):
        output_file = "%s/%s" % (self.output_dir, self.test_cases[case_id])
        solution = "%s/%s" % (self.sample_solution_dir,
        ...
        for line1, line2 in zip(sample_content, output_content):
            ...
            if line1.rstrip() != line2.rstrip():
                ok = False
                diff_output.append([
                    "Difference found in line %d" % line_num,
                    "sample: " + line1.rstrip('\n'),
                    "yours:  " + line2.rstrip('\n')
                ])
        ...
        if len(sample_content) > line_num:
            ...
            for idx in range(line_num, len(sample_content)):
                diff_output.append([
                    "Difference found in line %d" % (idx + 1),
        ...
        if len(output_content) > line_num:
            ...
            for idx in range(line_num, len(output_content)):
                diff_output.append([
                    "Difference found in line %d" % (idx + 1), "sample:",
    ...
    def run(self):
        for b_id in self.case_id_list:
            ...
            if ok:
                print(Colors.YELLOW + "Running test case: " + Colors.BLUE +
                      c_name + "  ==>  " + Colors.GREEN + "Pass!")
            else:
                print(Colors.YELLOW + "Running test case: " + Colors.BLUE +
                      c_name + "  ==>  " + Colors.RED + "Fail!")
...
def main():
    ...
    parser.add_argument("--parser",
                        help="Your parser to test.",
                        default="../src/parser")
    parser.add_argument(
        "--output_dir",
        help="Directory that stores the output content of your parser.",
        default="./outputs")
    parser.add_argument("--custom", action="store_true")
    parser.add_argument("--gen", action="store_true")
    parser.add_argument("--case_id",
                        help="ID of case to test. \
    ...
    onlyfiles = next(
        os.walk("./custom-cases"))[2]  #dir is your directory path as string
    top = 11 + len(onlyfiles)
    #print(top)
    ...
    if args.custom == False:
        ...
        g = Grader(parser=args.parser, output_dir=args.output_dir)
        #g.output_dir = "./custom-"
        g.test_case_dir = "./custom-cases"
        g.sample_solution_dir = "./custom-solutions"
        g.test_cases = {}
        for i in range(11, top):
            g.test_cases[i] = "{}".format(i)

    if args.gen:
        print("----GENERATING SOLUTIONS----")
        g.test_case_dir = "./custom-cases"
        g.sample_solution_dir = "./custom-solutions"
        g.output_dir = "./custom-solutions"
        for i in range(11, top + 1):
            g.test_cases[i] = "{}".format(i)
            #g.get_case_id_list(i)
            g.gen_output(i)
        exit(1)
